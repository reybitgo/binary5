...
function displayComprehensiveResults(PDO $pdo) {
    ...
    if (empty($usersWithAllBonuses)) {
        ...
        echo "\n🔍 DEBUG: Missing bonus types per user:\n";
        $stmt = $pdo->query(
        ...
        foreach ($stmt->fetchAll() as $user) {
            ...
            if (!empty($missing)) {
                echo "├── {$user['username']} (ID:{$user['id']}) missing: " . implode(", ", $missing) . "\n";
            }
        }
        
        // Check mentor bonus eligibility specifically
        echo "\n🔍 DEBUG: Mentor Bonus Eligibility Check:\n";
        ...
        if (empty($binaryEarners)) {
            echo "├── ❌ No binary bonuses earned - mentor bonuses can't trigger\n";
        } else {
            foreach ($binaryEarners as $earner) {
                $username = getUsernameById($earner['user_id'], $pdo);
                echo "├── ✅ {$username} (ID:{$earner['user_id']}) earned binary: $" . number_format($earner['total_binary'], 2) . "\n";
                
                // Check their descendants
                echo "│   └── Checking descendants for mentor bonus eligibility...\n";
                ...
                if (empty($descendants)) {
                    ...
                    foreach ($descendants as $desc) {
                        ...
                        if ($schedule) {
                            ...
                            echo "│   ├── L{$level}: {$descUsername} (ID:{$descId}) - {$status}\n";
                            echo "│   │   └── PVT: \${$pvt}/{$schedule['pvt_required']}, GVT: \${$gvt}/{$schedule['gvt_required']} ({$schedule['package_name']})\n";
                        } else {
                            echo "│   ├── L{$level}: {$descUsername} (ID:{$descId}) - ❌ NO PACKAGE FOUND\n";
                        }
                    }
                }
            }

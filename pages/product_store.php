...
<div class="bg-white shadow rounded-lg p-6">
    ...
    <!-- Affiliate banner -->
    <?php if ($aff && $aff !== $uid): ?>
        <?php $referrer = getUsernameById($aff, $pdo); ?>
        <?php if ($product_id > 0): ?>
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                <p class="text-sm text-green-800">
                    <strong>Affiliate Link Active!</strong>
                    You're shopping via <?= htmlspecialchars($referrer) ?>'s link – they'll earn commission if you buy.
                </p>
            </div>
        <?php else: ?>
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                <p class="text-sm text-blue-800">
                    <strong>Browsing via affiliate:</strong> <?= htmlspecialchars($referrer) ?>
                </p>
            </div>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <?php foreach ($products as $p): ?>
        <div class="border rounded-lg p-4 flex flex-col <?= ($product_id && $product_id == $p['id']) ? 'ring-2 ring-green-400 bg-green-50' : '' ?>">
            <?php if ($p['image_url']): ?>
            ...
            <?php if ($p['affiliate_rate'] > 0): ?>
                <div class="text-xs text-purple-600 mb-2">Affiliate commission: <?= $p['affiliate_rate'] ?>%</div>
            <?php endif; ?>
            ...
            <?php if ($user['balance'] >= $p['final_price']): ?>
                <form method="post" action="dashboard.php" class="mt-auto">
                    ...
                    <input type="hidden" name="affiliate_id" value="<?= $aff ?>">

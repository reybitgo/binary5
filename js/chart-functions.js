...
function renderOrgChart(rootData, containerId, resetId, expandId, collapseId) {
    ...
    if (resetBtn) resetBtn.onclick = () => { centerOnRoot(); };
    if (expandBtn) expandBtn.onclick = () => { expandAll(root); update(root); };
    if (collapseBtn) collapseBtn.onclick = () => { collapseAll(root); update(root); };

...
// Adjusted scenario with additional users on right leg to trigger admin binary, enabling mentor bonuses for descendants.
...
function setupTestData(PDO $pdo) {
    ...
    $users = [
        ...
        ['leader', 'pass123', 1, 1, 'left', 'user'],       // ID: 2, Level 1 under admin (left)
        ...
        ['right_leader', 'pass123', 1, 1, 'right', 'user'], // ID: 9, Level 1 under admin (right)
        ['right_child1', 'pass123', 9, 9, 'left', 'user'],  // ID: 10, Level 2 under right_leader (left)
        ['right_child2', 'pass123', 9, 9, 'right', 'user']  // ID: 11, Level 2 under right_leader (right)
    ];
    ...
    // Insert packages if not present (assuming IDs 1-3), adjusted daily_max higher for more pairing
    $pdo->exec("DELETE FROM packages");
    $pdo->exec("ALTER TABLE packages AUTO_INCREMENT = 1");
    $packages = [
        ['Starter', 25.00, 25, 100, 0.2000, 0.1000],  // ID:1, daily_max 100
        ['Pro', 50.00, 50, 200, 0.2000, 0.1000],      // ID:2, daily_max 200
        ['Elite', 100.00, 100, 500, 0.2000, 0.1000]   // ID:3, daily_max 500 for testing larger bonuses
    ];
    ...
    // Insert leadership schedules (adjusted lower requirements for testing)
    $pdo->exec("DELETE FROM package_leadership_schedule");
    $leadership_schedules = [
        // Starter (ID:1) - Lowered req
        [1, 1, 25, 100, 0.050], [1, 2, 50, 200, 0.040], [1, 3, 100, 400, 0.030], [1, 4, 150, 800, 0.020], [1, 5, 250, 1600, 0.010],
        // Pro (ID:2)
        [2, 1, 50, 200, 0.060], [2, 2, 100, 400, 0.050], [2, 3, 150, 800, 0.030], [2, 4, 250, 1600, 0.020], [2, 5, 500, 3200, 0.010],
        // Elite (ID:3)
        [3, 1, 100, 400, 0.070], [3, 2, 200, 800, 0.060], [3, 3, 300, 1600, 0.050], [3, 4, 500, 3200, 0.040], [3, 5, 1000, 6400, 0.030]
    ];
    ...
    // Insert mentor schedules (adjusted lower requirements for testing)
    $pdo->exec("DELETE FROM package_mentor_schedule");
    $mentor_schedules = [
        // Starter (ID:1)
        [1, 1, 25, 100, 0.020], [1, 2, 50, 200, 0.018], [1, 3, 100, 400, 0.015], [1, 4, 150, 800, 0.012], [1, 5, 250, 1600, 0.010],
        // Pro (ID:2)
        [2, 1, 50, 200, 0.030], [2, 2, 100, 400, 0.025], [2, 3, 150, 800, 0.020], [2, 4, 250, 1600, 0.015], [2, 5, 500, 3200, 0.010],
        // Elite (ID:3)
        [3, 1, 100, 400, 0.040], [3, 2, 200, 800, 0.035], [3, 3, 300, 1600, 0.030], [3, 4, 500, 3200, 0.025], [3, 5, 1000, 6400, 0.020]
    ];
    ...
    echo "✅ Test data ready (11 users, packages, and schedules)\n";
...
function testAllBonusesScenario(PDO $pdo) {
    ...
    echo "\nSetting up qualifications with multiple package purchases on left side...\n";
    
    // Admin (1) buys Elite 4 times (PVT=400, meets higher levels)
    for ($i = 0; $i < 4; $i++) {
        simulatePackagePurchase($pdo, 1, 3);
    }
    
    // Leader (2) buys Elite 4 times (PVT=400)
    for ($i = 0; $i < 4; $i++) {
        simulatePackagePurchase($pdo, 2, 3);
    }
    
    // Child1 (3) buys Elite 3 times (PVT=300)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 3, 3);
    }
    
    // Child2 (4) buys Elite 3 times (PVT=300, triggers leader binary)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 4, 3);
    }
    
    // Grand1 (5) buys Elite 3 times (PVT=300)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 5, 3);
    }
    
    // Grand2 (6) buys Elite 3 times (PVT=300, triggers child1 binary)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 6, 3);
    }
    
    // Great1 (7) buys Elite 2 times (PVT=200, triggers propagation)
    for ($i = 0; $i < 2; $i++) {
        simulatePackagePurchase($pdo, 7, 3);
    }
    
    // Great2 (8) buys Elite 2 times (PVT=200, triggers grand1 binary)
    for ($i = 0; $i < 2; $i++) {
        simulatePackagePurchase($pdo, 8, 3);
    }
    
    echo "\nSetting up right side to trigger admin binary and mentor bonuses...\n";
    
    // Right_leader (9) buys Elite 4 times (PVT=400)
    for ($i = 0; $i < 4; $i++) {
        simulatePackagePurchase($pdo, 9, 3);
    }
    
    // Right_child1 (10) buys Elite 3 times (PVT=300)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 10, 3);
    }
    
    // Right_child2 (11) buys Elite 3 times (PVT=300, triggers right_leader binary and admin binary)
    for ($i = 0; $i < 3; $i++) {
        simulatePackagePurchase($pdo, 11, 3);
    }
